--[[
        ...
        V3xadoUI Interface Suite
        by V3xado

]]

if debugX then
        warn('Initialising V3xadoUI')
end
...
local requestsDisabled = true --getgenv and getgenv().DISABLE_V3XADO_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.68"
local V3xadoUIFolder = "V3xadoUI"
local ConfigurationFolder = V3xadoUIFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
        General = {
                -- if needs be in order just make getSetting(name)
                v3xadoOpen = {Type = 'bind', Value = 'K', Name = 'V3xadoUI Keybind'},
                -- buildwarnings
                -- v3xadoprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.v3xadoOpen"] = "J"
...
local function callSafely(func, ...)
        if func then
                ...
                if not success then
                        warn("V3xadoUI | Function failed with error: ", result)
                        return false
                else
                        return result
                end
...
local function loadSettings()
        ...
        local success, result = pcall(function()
                task.spawn(function()
                        if callSafely(isfolder, V3xadoUIFolder) then
                                if callSafely(isfile, V3xadoUIFolder..'/settings'..ConfigurationExtension) then
                                        file = callSafely(readfile, V3xadoUIFolder..'/settings'..ConfigurationExtension)
                                end
                        end
                        ...
                        if useStudio then
                                file = [[
                {"General":{"v3xadoOpen":{"Value":"K","Type":"bind","Name":"V3xadoUI Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"V3xadoUI Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
        ]]
                        end
        ...
        if not success then 
                if writefile then
                        warn('V3xadoUI had an issue accessing configuration saving capability.')
                end
        end
end
...
local V3xadoUI = useStudio and script.Parent:FindFirstChild('V3xadoUI') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local v3xadoDestroyed = false -- True when V3xadoUILibrary:Destroy() is called

repeat
        if V3xadoUI:FindFirstChild('Build') and V3xadoUI.Build.Value == InterfaceBuild then
                correctBuild = true
                break
        end
        ...
        if not warned then
                warn('V3xadoUI | Build Mismatch')
                print('V3xadoUI may encounter issues as you are running an incompatible interface version ('.. ((V3xadoUI:FindFirstChild('Build') and V3xadoUI.Build.Value) or 'No Build') ..').\n\nThis version of V3xadoUI is intended for interface build '..InterfaceBuild..'.')
                warned = true
        end

        toDestroy, V3xadoUI = V3xadoUI, useStudio and script.Parent:FindFirstChild('V3xadoUI') or game:GetObjects("rbxassetid://10804731440")[1]
        if toDestroy and not useStudio then toDestroy:Destroy() end

        buildAttempts = buildAttempts + 1
until buildAttempts >= 2

V3xadoUI.Enabled = false

if gethui then
        V3xadoUI.Parent = gethui()
elseif syn and syn.protect_gui then 
        syn.protect_gui(V3xadoUI)
        V3xadoUI.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
        V3xadoUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
        for _, Interface in ipairs(gethui():GetChildren()) do
                if Interface.Name == V3xadoUI.Name and Interface ~= V3xadoUI then
                        Interface.Enabled = false
                        Interface.Name = "V3xadoUI-Old"
                end
        end
elseif not useStudio then
...
if V3xadoUI.AbsoluteSize.X < minSize.X and V3xadoUI.AbsoluteSize.Y < minSize.Y then
...
local Main = V3xadoUI.Main
local MPrompt = V3xadoUI:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = V3xadoUI:FindFirstChild('Drag')
...
V3xadoUI.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/V3xadoLTC/V3xadoUI/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = V3xadoUI.Notifications
local keybindConnections = {} -- For storing keybind connections to disconnect when V3xadoUI is destroyed

local SelectedTheme = V3xadoUILibrary.Theme.Default

local function ChangeTheme(Theme)
        if typeof(Theme) == 'string' then
                SelectedTheme = V3xadoUILibrary.Theme[Theme]
        elseif typeof(Theme) == 'table' then
                SelectedTheme = Theme
        end

        V3xadoUI.Main.BackgroundColor3 = SelectedTheme.Background
        V3xadoUI.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
        V3xadoUI.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
        V3xadoUI.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

        V3xadoUI.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
        V3xadoUI.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
        V3xadoUI.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
        if Topbar:FindFirstChild('Settings') then
                V3xadoUI.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
                V3xadoUI.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
        end
        ...
        for _, text in ipairs(V3xadoUI:GetDescendants()) do
...
local function getAssetUri(id: any): string
        ...
        elseif type(id) == "string" and not Icons then
                warn("V3xadoUI | Cannot use Lucide icons as icons library is not loaded")
        else
                warn("V3xadoUI | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
        end
        return assetUri
end
...
local function LoadConfiguration(Configuration)
        ...
        if not success then warn('V3xadoUI had an issue decoding the configuration file, please try delete the file and reopen V3xadoUI.') return end

        -- Iterate through current UI elements' flags
        for FlagName, Flag in pairs(V3xadoUILibrary.Flags) do
                ...
                if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
                        ...
                        warn("V3xadoUI | Unable to find '"..FlagName.. "' in the save file.")
                        print("The error above may not be an issue if new elements have been added or not been set values.")
                        --V3xadoUILibrary:Notify({Title = "V3xadoUI Flags", Content = "V3xadoUI was unable to find '"..FlagName.. "' in the save file.", Image = 3944688398})
                end
        end
...
local function SaveConfiguration()
        ...
        local Data = {}
        for i, v in pairs(V3xadoUILibrary.Flags) do
...
function V3xadoUILibrary:Notify(data) -- action e.g open messages
        task.spawn(function()
                ...
                if data.Actions then
                        warn('V3xadoUI | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
        ...
        if notify then
                if useMobilePrompt then 
                        V3xadoUILibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
                else
                        V3xadoUILibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "v3xadoOpen")}.`, Duration = 7, Image = 4400697855})
                end
        end
...
local function saveSettings() -- Save settings to config file
        ...
        if success then
                ...
                callSafely(writefile, V3xadoUIFolder..'/settings'..ConfigurationExtension, encoded)
        end
end
...
local function createSettings(window)
        ...
        local newTab = window:CreateTab('V3xadoUI Settings', 0, true)

        if TabList['V3xadoUI Settings'] then
                TabList['V3xadoUI Settings'].LayoutOrder = 1000
        end

        if Elements['V3xadoUI Settings'] then
                Elements['V3xadoUI Settings'].LayoutOrder = 1000
        end
...
function V3xadoUILibrary:CreateWindow(Settings)
        if V3xadoUI:FindFirstChild('Loading') then
                if getgenv and not getgenv().v3xadoCached then
                        V3xadoUI.Enabled = true
                        V3xadoUI.Loading.Visible = true

                        task.wait(1.4)
                        V3xadoUI.Loading.Visible = false
                end
        end

        if getgenv then getgenv().v3xadoCached = true end

        if not correctBuild and not Settings.DisableBuildWarnings then
                task.delay(3, 
                        function() 
                                V3xadoUILibrary:Notify({Title = 'Build Mismatch', Content = 'V3xadoUI may encounter issues as you are running an incompatible interface version ('.. ((V3xadoUI:FindFirstChild('Build') and V3xadoUI.Build.Value) or 'No Build') ..').\n\nThis version of V3xadoUI is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})          
                        end)
        end
        ...
        LoadingFrame.Title.Text = Settings.LoadingTitle or "V3xadoUI"
        LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

        if Settings.LoadingTitle ~= "V3xadoUI Interface Suite" then
                LoadingFrame.Version.Text = "V3xadoUI UI"
        end
        ...
        if not Settings.DisableV3xadoUIPrompts then
                task.spawn(function()
                        while true do
                                task.wait(math.random(180, 600))
                                V3xadoUILibrary:Notify({
                                        Title = "V3xadoUI Interface",
        ...
        if Settings.Discord and Settings.Discord.Enabled and not useStudio then
                ensureFolder(V3xadoUIFolder.."/Discord Invites")

                if callSafely(isfile, V3xadoUIFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
                        if requestFunc then
                                pcall(function()
                                        requestFunc({
                        ...
                        if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
                                callSafely(writefile, V3xadoUIFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"V3xadoUI RememberJoins is true for this invite, this invite will not ask you to join again")
                        end
                end
        end
        ...
        if (Settings.KeySystem) then
                ...
                ensureFolder(V3xadoUIFolder.."/Key System")
                ...
                if Settings.KeySettings.GrabKeyFromSite then
                        for i, Key in ipairs(Settings.KeySettings.Key) do
                                ...
                                if not Success then
                                        print("V3xadoUI | "..Key.." Error " ..tostring(Response))
                                        warn('Check docs.sirius.menu for help with V3xadoUI specific development.')
                                end
                        end
                end
                ...
                if callSafely(isfile, V3xadoUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
                        for _, MKey in ipairs(Settings.KeySettings.Key) do
                                local savedKeys = callSafely(readfile, V3xadoUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
                        ...
                        KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
                                ...
                                if KeyFound then 
                                        ...
                                        if Settings.KeySettings.SaveKey then
                                                callSafely(writefile, V3xadoUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
                                                V3xadoUILibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
                                        end
                                else
        ...
        function Window:CreateTab(Name, Image, Ext)
                ...
                function Tab:CreateButton(ButtonSettings)
                        ...
                        Button.Interact.MouseButton1Click:Connect(function()
                                ...
                                -- Prevents animation from trying to play if the button's callback called V3xadoUILibrary:Destroy()
                                if v3xadoDestroyed then
                                        return
                                end
                                if not Success then
                                        ...
                                        print("V3xadoUI | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
                                        warn('Check docs.sirius.menu for help with V3xadoUI specific development.')
                                        task.wait(0.5)
                                        Button.Title.Text = ButtonSettings.Name
                ...
                function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
                        ...
                        ColorPicker.Interact.MouseButton1Down:Connect(function()
                                ...
                                if not opened then
                                        ...
                                        TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= V3xadoUILibrary.Theme.Default and 0.25 or 0.1}):Play()
                        ...
                        if Settings.ConfigurationSaving then
                                if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
                                        V3xadoUILibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
                                end
                        end
                        ...
                        V3xadoUI.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
                                for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
                                        if rgbinput:IsA("Frame") then
                                                rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
                                                rgbinput.UIStroke.Color = SelectedTheme.InputStroke
                ...
                function Tab:CreateInput(InputSettings)
                        ...
                        Input.InputFrame.InputBox.FocusLost:Connect(function()
                                ...
                                if not Success then
                                        ...
                                        print("V3xadoUI | "..InputSettings.Name.." Callback Error " ..tostring(Response))
                                        warn('Check docs.sirius.menu for help with V3xadoUI specific development.')
                                        task.wait(0.5)
                                        Input.Title.Text = InputSettings.Name
                        ...
                        if Settings.ConfigurationSaving then
                                if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
                                        V3xadoUILibrary.Flags[InputSettings.Flag] = InputSettings
                                end
                        end
                ...
                function Tab:CreateDropdown(DropdownSettings)
                                ...
                                for _, Option in ipairs(DropdownSettings.Options) do
                                        ...
                                        DropdownOption.Interact.MouseButton1Click:Connect(function()
                                                ...
                                                if not Success then
                                                        ...
                                                        print("V3xadoUI | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
                                                        warn('Check docs.sirius.menu for help with V3xadoUI specific development.')
                                                        task.wait(0.5)
                                                        Dropdown.Title.Text = DropdownSettings.Name
                                                        TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
                        ...
                        if Settings.ConfigurationSaving then
                                if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
                                        V3xadoUILibrary.Flags[DropdownSettings.Flag] = DropdownSettings
                                end
                        end
                ...
                function Tab:CreateKeybind(KeybindSettings)
                                ...
                                elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
                                        ...
                                        if not KeybindSettings.HoldToInteract then
                                                ...
                                                if not Success then
                                                        ...
                                                        print("V3xadoUI | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
                                                        warn('Check docs.sirius.menu for help with V3xadoUI specific development.')
                                                        task.wait(0.5)
                                                        Keybind.Title.Text = KeybindSettings.Name
                                                        TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
                        ...
                        if Settings.ConfigurationSaving then
                                if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
                                        V3xadoUILibrary.Flags[KeybindSettings.Flag] = KeybindSettings
                                end
                        end
                ...
                function Tab:CreateToggle(ToggleSettings)
                        ...
                        if SelectedTheme ~= V3xadoUILibrary.Theme.Default then
                        ...
                        Toggle.Interact.MouseButton1Click:Connect(function()
                                ...
                                if not Success then
                                        ...
                                        print("V3xadoUI | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
                                        warn('Check docs.sirius.menu for help with V3xadoUI specific development.')
                                        task.wait(0.5)
                                        Toggle.Title.Text = ToggleSettings.Name
                                        TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
                        ...
                        if not ToggleSettings.Ext then
                                if Settings.ConfigurationSaving then
                                        if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
                                                V3xadoUILibrary.Flags[ToggleSettings.Flag] = ToggleSettings
                                        end
                                end
                        end
                ...
                function Tab:CreateSlider(SliderSettings)
                                        ...
                                        if SLDragging then
                                                ...
                                                if SliderSettings.CurrentValue ~= NewValue then
                                                        ...
                                                        if not Success then
                                                                ...
                                                                print("V3xadoUI | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
                                                                warn('Check docs.sirius.menu for help with V3xadoUI specific development.')
                                                                task.wait(0.5)
                                                                Slider.Title.Text = SliderSettings.Name
                                                                TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
                        ...
                        if Settings.ConfigurationSaving then
                                if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
                                        V3xadoUILibrary.Flags[SliderSettings.Flag] = SliderSettings
                                end
                        end
        ...
        function Window.ModifyTheme(NewTheme)
                ...
                if not success then
                        V3xadoUILibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
                else
                        V3xadoUILibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
                end
        end
        ...
        if not success then warn('V3xadoUI had an issue creating settings.') end
...
function V3xadoUILibrary:SetVisibility(visibility: boolean)
        setVisibility(visibility, false)
end

function V3xadoUILibrary:IsVisible(): boolean
        return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function V3xadoUILibrary:Destroy()
        v3xadoDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
        Topbar.Settings.MouseButton1Click:Connect(function()
                task.spawn(function()
                        ...
                        Elements.UIPageLayout:JumpTo(Elements['V3xadoUI Settings'])
                end)
        end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
        if (input.KeyCode == Enum.KeyCode[getSetting("General", "v3xadoOpen")]) and not processed then
                if Debounce then return end
                if Hidden then
                        Hidden = false
                        Unhide()
...
function V3xadoUILibrary:LoadConfiguration()
        local config
        ...
        if CEnabled then
                ...
                local success, result = pcall(function()
                        ...
                        if isfile then 
                                ...
                                notified = true
                                V3xadoUILibrary:Notify({Title = "V3xadoUI Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
                        end
                end)

                if success and loaded and not notified then
                        V3xadoUILibrary:Notify({Title = "V3xadoUI Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
                elseif not success and not notified then
                        warn('V3xadoUI Configurations Error | '..tostring(result))
                        V3xadoUILibrary:Notify({Title = "V3xadoUI Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
                end
        end
...
if useStudio then
        ...
        --local Window = V3xadoUILibrary:CreateWindow({
        ...
        --              FileName = "Key", -- It is recommended to use something unique as other scripts using V3xadoUI may overwrite your key file
        --              SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
        --              GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like V3xadoUI to get the key from
        ...
        ----V3xadoUILibrary:Notify({Title = "V3xadoUI Interface", Content = "Welcome to V3xadoUI. These - are the brand new notification design for V3xadoUI, with custom sizing and V3xadoUI calculated wait times.", Image = 4483362458})
        ...
        --local thoptions = {}
        --for themename, theme in pairs(V3xadoUILibrary.Theme) do
...
task.delay(4, function()
        V3xadoUILibrary.LoadConfiguration()
